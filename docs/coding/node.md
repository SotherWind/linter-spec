...
:::tip
`Node.js` 规范主要包含编码风格、安全规范、最佳实践等几个部分，目的是给业务同学提供研发过程中的实质性规范和指导。其中编码风格基于 [eslint-plugin-n](https://github.com/eslint-community/eslint-plugin-n)（社区维护的 `eslint-plugin-node` 继任者）。
:::

## 1. 编码风格

- 1.1.【推荐】使用 `Node.js` 内置的全局变量。`eslint`: [n/prefer-global](https://github.com/eslint-community/eslint-plugin-n/tree/master/docs/rules/prefer-global)
...
- 1.2.【推荐】使用模块内支持的 `promises` API。`eslint`: [n/prefer-promises](https://github.com/eslint-community/eslint-plugin-n/tree/master/docs/rules/prefer-promises)
...
- 1.3.【推荐】如无特殊需求，模块引用声明放在文件顶端，注意引用顺序。`eslint`: [import/order](https://github.com/un-ts/eslint-plugin-import-x/blob/master/docs/rules/order.md)
