...
- header（首行）：只有一行，不超过 100 个字符（`header-max-length`）
- body：每行不超过 100 个字符（`body-max-line-length`）
- footer：每行不超过 100 个字符（`footer-max-line-length`）

传统约定常推荐 header ≤ 50、body/footer ≤ 72 作为书写习惯，但本工具（`@linter-spec/commitlint-config`）实际卡点放宽到了 100。
...
目前互联网和社区中流传最广泛的一个分支模型 [Git Flow](https://github.com/nvie/gitflow) 出自 [a-successful-git-branching-model](https://nvie.com/posts/a-successful-git-branching-model/) 这篇十年前的文章，文章作者 Vincent Driessen 在 2020 年三月份的时候已经公开表示，该分支模型已经不适用于现如今持续交付的软件工程方式，推荐在持续交付的软件工程中使用更简单的 [GitHub Flow](https://docs.github.com/en/get-started/using-github/github-flow) 模型。
...
[stackoverflow-git-msg]: https://stackoverflow.com/questions/3580013/should-i-use-past-or-present-tense-in-git-commit-messages
[karma-git-msg]: https://karma-runner.github.io/0.13/dev/git-commit-msg.html
